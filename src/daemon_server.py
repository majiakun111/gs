...
def github_job(subscription_manager, github_client, report_generator, notifier, days):
    ...
    for repo in subscriptions:
        ...
        markdown_file_path = github_client.export_progress_by_date_range(owner, repo_name, days)
        # 从Markdown文件自动生成进展简报
        report, report_file_path = report_generator.generate_report(markdown_file_path)
...
def main():
    ...
    llm = LLM(config.openai_api_key)  # 创建语言模型实例
